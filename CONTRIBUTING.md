# Contributing to Canvas LMS Browser Automation
...
- **Broken selectors**: Canvas LMS updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Canvas LMS instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.CANVAS_URL}/path/to/section`);
...
When you find working selectors for Canvas LMS, document them:

```javascript
// Canvas LMS selectors (verified YYYY-MM-DD)
...
Canvas LMS updates its UI periodically. When selectors break:
