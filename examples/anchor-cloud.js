/**
 * anchor-cloud.js — AnchorBrowser cloud example for Canvas LMS
 *
...
const { login_canvas, create_course } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Canvas LMS automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_canvas(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_course(page, {});
