...
This project gives you a **complete browser automation scaffold** for Canvas LMS using Puppeteer (self-hosted, open source) or [AnchorBrowser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system requires **MFA** (Duo / Okta MFA). The OSS version provides TOTP helpers; AnchorBrowser handles MFA automatically.
...
*Built with ❤️ for developers who need to automate Canvas LMS without wrestling with its API limitations. Powered by [AnchorBrowser](https://anchorbrowser.io) for cloud-scale automation.*
